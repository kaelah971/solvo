...
export default async function ClaimsPage() {
  const page = await requireDashboardPageContext(await headers(), "claims");
  if (!page.ok) return <DashboardUnavailable />;

  const model = await buildClaimListPageModel(page.repo, page.ctx);
      ...
      {model.empty ? (
        ...
        <ul className="mt-6 overflow-hidden rounded-xl border border-line bg-[#191919]">
          {model.items.map((item) => (
            <li key={item.view.claimId} className="hairline-top px-5 py-4 transition-colors hover:bg-white/[0.025] sm:px-6">
              <Link
